...
public class MyPanel extends JPanel {
	...
	public MyPanel(Person person, int dWidth, int dHeight, Canvas canvas) {
		...
		/*
		 * diamond.translate(new Vec(55,100)); diamond.setDrawingArea(200, 200);
		 * diamond.translate(new Vec(55,100)); star.translate(new Vec(110,0));
		 * square.translate(new Vec(200,0)); oldWall.translate(new Vec(150,100));
		 * 
		 * // Test printRGB("Light  Gray RGB: " , Color.LIGHT_GRAY);
		 * printRGB("       Gray RGB: " , Color.GRAY); printRGB("Dark   Gray RGB: " ,
		 * Color.DARK_GRAY); printRGB("Darker Gray RGB: " , Color.GRAY.darker());
		 * printRGB("Custom Gray RGB: " , new Color(100,100,100)); printRGB("Blue RGB: "
		 * , Color.BLUE); printRGB("Brighter Blue RGB: " , Color.BLUE.darker());
		 * printRGB("White RGB: " , Color.WHITE);
		 * 
		 * // Test byte bigbyte = 127; System.out.println("bigbyte " + bigbyte); bigbyte
		 * += 2; System.out.println("bigbyte + 2 " + bigbyte);
	 ...
	 * 
	 * int dy=0,dx=0,x=0,y=0; int rowH = 0;
	 * 
	 * Rectangle bnds;
	 * 
	 ...
	 * 
	 * bnds = obj.getBounds(); rowH = Math.max(rowH, bnds.height);
	 * 
	 * // New row? if ((bnds.width + x) > getWidth()) { g2d.translate(-x, rowH);
	 * rowH=0; x=0; } obj.draw(g2d); x += bnds.width; g2d.translate(bnds.width, 0);
	 * 
	 ...
	 * public void paintComponent(Graphics g) { super.paintComponent(g);
	 * 
	 * g.setColor(Color.BLUE); g.fillOval(0, 0, 50, 50); g.setColor(lightBlue);
	...
	public void paintComponent(Graphics g) {
		...
		// test some shapes
		/*
		 * para.draw((Graphics2D)g); diamond.draw((Graphics2D)g);
		 * star.draw((Graphics2D)g); square.draw((Graphics2D)g);
		 * oldWall.draw((Graphics2D)g); wall.draw((Graphics2D)g); wall.translate(tx);
		 * wall.draw((Graphics2D)g); wall.translate(ty); wall.draw((Graphics2D)g);
		 * wall.translate(negTx); wall.draw((Graphics2D)g);
		 */
		...
		if (Controller.getInstance().getShowGrid()) {
